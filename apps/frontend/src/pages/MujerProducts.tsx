...
import { getDynamicAttributesForGender, DynamicAttribute } from "@/services/dynamicAttributes";
...
const MujerProducts: React.FC = () => {
  ...
  useEffect(() => {
    const loadData = async () => {
      try {
        setLoading(true);
        const [productsData, filtersData, dynamicAttrs] = await Promise.all([
          getProductsByGender("mujer"),
          getDynamicFiltersForGender("mujer"),
          getDynamicAttributesForGender("mujer"),
        ]);
        ...
        // Actualizar rangos de precio iniciales
        setFilters((prev) => ({
          ...prev,
        ...
        console.log("✅ Mujer - Productos cargados:", productsData.length);
        console.log("✅ Mujer - Filtros dinámicos:", filtersData);
        console.log("✅ Mujer - Atributos dinámicos:", dynamicAttrs);
      } catch (error) {
  ...
  const applyFilters = useMemo(() => {
    ...
    dynamicAttributes.forEach((attr) => {
      ...
      if (filterValues && filterValues.length > 0) {
        ...
        if (attr.slug === "frame_size") {
          filtered = filtered.filter(
            (p) =>
              (p as DbProduct & { frame_size?: string }).frame_size &&
              filterValues.includes((p as DbProduct & { frame_size?: string }).frame_size!)
          );
        } else if (attr.slug === "frame_material") {
          filtered = filtered.filter(
            (p) =>
              (p as DbProduct & { frame_material?: string }).frame_material &&
              filterValues.includes((p as DbProduct & { frame_material?: string }).frame_material!)
          );
        }
      }
    });
    ...
    return filtered;
  }, [products, filters, sortBy, dynamicAttributes]);
  ...
  const handleAddToCart = (product: ProductWithBrand) => {
    ...
    const opticalProduct: OpticalProduct = {
      ...product,
      brand: product.brand
        ? {
            id: product.brand.id,
  ...
  return (
    <>
      <SEO
        title="Lentes para Mujeres | Full Vision"
        description="Descubre nuestra colección de lentes para mujeres con descuentos especiales. Filtros por marca, precio, talla y material."
      ...
      <div className="container mx-auto px-4 py-8">
        ...
        <div className="mb-8">
          ...
          <nav className="flex items-center gap-2 text-sm text-muted-foreground mb-4">
            ...
            <ChevronRight className="h-4 w-4" />
            <span className="text-foreground font-medium">Mujer</span>
          </nav>

          <div className="flex items-center justify-between">
            <div>
              <h1 className="text-3xl font-bold mb-2">Lentes para Mujeres</h1>
              <p className="text-muted-foreground">Encuentra los lentes perfectos para mujeres con descuentos especiales</p>
            </div>
